-- Migration script: brings ALL existing tenant schemas up to date with current entity definitions.
-- Safe to run multiple times (uses IF NOT EXISTS / IF EXISTS guards).
-- Run this after updating create_tenant_schema() to ensure existing tenants match new tenants.
...
BEGIN
    ...
    EXECUTE format('
        CREATE INDEX IF NOT EXISTS idx_lead_activities_lead ON %I.lead_activities (lead_id)
    ', t_schema);
    EXECUTE format('
        CREATE INDEX IF NOT EXISTS idx_lead_activities_created_at ON %I.lead_activities (created_at)
    ', t_schema);
