...
@Entity
@Table(
        name = "apipay_invoices",
        uniqueConstraints = {
                @UniqueConstraint(name = "uq_apipay_invoices_merchant_invoice", columnNames = {"merchant_invoice_id"}),
                @UniqueConstraint(name = "uq_apipay_invoices_sub_month", columnNames = {"subscription_id", "payment_month"})
        },
        indexes = {
                @Index(name = "idx_apipay_invoices_status", columnList = "status"),
                @Index(name = "idx_apipay_invoices_student", columnList = "student_id")
        }
)
@Getter
@Setter
...
@NoArgsConstructor
@AllArgsConstructor
public class ApiPayInvoice extends BaseEntity {

    @Column(nullable = false, length = 20)
    @Builder.Default
    private String provider = "APIPAY";
    ...
    @Column(nullable = false, length = 20)
    @Builder.Default
    private ApiPayInvoiceStatus status = ApiPayInvoiceStatus.CREATED;
