...
public class AttendanceStatusConfigController {
    ...
    @GetMapping
    @PreAuthorize("hasAnyRole('TENANT_ADMIN','MANAGER','ADMIN','RECEPTIONIST','TEACHER') or hasAuthority('SETTINGS_VIEW') or hasAuthority('LESSONS_VIEW') or hasAuthority('LESSONS_MARK_ATTENDANCE')")
    ...
    @PreAuthorize("hasAnyRole('TENANT_ADMIN', 'ADMIN') or hasAuthority('SETTINGS_EDIT')")
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyRole('TENANT_ADMIN', 'ADMIN') or hasAuthority('SETTINGS_EDIT')")
    @Operation(summary = "Delete an attendance status config")
